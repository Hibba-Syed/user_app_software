...
class ReceivedScreenOrderScreenState extends State<ReceivedScreenOrderScreen> {
  ...
  Future<void> init() async {
    ...
    if (mIsUpdate) {
      picUpController.text = DateFormat('yyyy-MM-dd HH:mm:ss').format(
          DateTime.parse(widget.orderData!.pickupDatetime.validate().isEmpty
  ...
  saveDelivery() async {
    ...
    await updateOrder(
      ...
      deliveryReceiverName: receiverNameController.text,
      clientName:
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.orderData!.status == ORDER_DEPARTED
                    ...
                    children: [
                      ...
                      if (widget.orderData!.status == ORDER_DEPARTED)
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(language.deliveryDatetime,
                                style: boldTextStyle()),
                            8.height,
                            AppTextField(
                              readOnly: true,
                      ...
                      Text(language.deliveryTimeSignature,
                              style: boldTextStyle())
