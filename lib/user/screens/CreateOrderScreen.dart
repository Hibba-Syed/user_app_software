...
class CreateOrderScreenState extends State<CreateOrderScreen> {
  ...
  getAppSettingApiCall() async {
    await getAppSetting().then((value) {
      carryPackagesCharge = num.parse(value.carryPackagesCharges! != null
                  ...
                  onTap: () async {
                    ...
                    if (response != null) {
                      pickAddressCont.text =
                          mData.description ?? "";
                      pickLat = response.result!
                          .geometry!.location!.lat
                          .toString();
                      pickLong = response.result!
  ...
  Widget createOrderWidget5() {
    return Column(
      ...
      children: [
        ...
        if (!anotherDeliver3Msg.isEmptyOrNull)
          Padding(
              padding: EdgeInsets.only(top: 8, left: 8),
              child: Text(
                anotherDeliver3Msg.validate(),
                style: secondaryTextStyle(color: Colors.red),
              )),
        if (deliverPredictionList.isNotEmpty)
          ListView.builder(
  ...
  Widget createOrderWidget6() {
    return Column(
      ...
      children: [
        ...
        if (!anotherDeliver4Msg.isEmptyOrNull)
          Padding(
              padding: EdgeInsets.only(top: 8, left: 8),
              child: Text(
                anotherDeliver4Msg.validate(),
                style: secondaryTextStyle(color: Colors.red),
              )),
        if (deliverPredictionList.isNotEmpty)
          ListView.builder(
  ...
  Widget createOrderWidget7() {
    return Column(
      ...
      children: [
        ...
        if (!anotherDeliver5Msg.isEmptyOrNull)
          Padding(
              padding: EdgeInsets.only(top: 8, left: 8),
              child: Text(
                anotherDeliver5Msg.validate(),
                style: secondaryTextStyle(color: Colors.red),
              )),
        if (deliverPredictionList.isNotEmpty)
          ListView.builder(
