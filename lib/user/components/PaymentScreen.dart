...
import 'package:mighty_delivery/main/models/iyzico_payment_model.dart';
...
import 'package:pay/pay.dart' as gpay;
...
class PaymentScreen extends StatefulWidget {
  ...
  PaymentScreen({required this.totalAmount,this.isWallet,this.orderId});
...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  List<gpay.PaymentItem> _paymentItems = [];
  ...
  void initState() {
    ...
    _paymentItems = [
      gpay.PaymentItem(
        ...
        status: gpay.PaymentItemStatus.final_price,
      )
    ];
  }
  ...
  Future<void> init() async {
    ...
    if (paymentGatewayList.any((element) =>
    element.type == PAYMENT_TYPE_STRIPE)) {
    ...
    if (paymentGatewayList.any((element) =>
    element.type == PAYMENT_TYPE_PAYSTACK)) {
      plugin.initialize(publicKey: payStackPublicKey.validate());
    }
    if (paymentGatewayList.any((element) =>
    element.type == PAYMENT_TYPE_RAZORPAY)) {
      _razorpay = Razorpay();
      ...
      if (paymentGatewayList.isNotEmpty) {
        paymentGatewayList.forEach((element) {
          if (element.type == PAYMENT_TYPE_STRIPE) {
            stripPaymentKey =
            element.isTest == 1 ? element.testValue!.secretKey : element
                .liveValue!.secretKey;
            stripPaymentPublishKey =
            element.isTest == 1 ? element.testValue!.publishableKey : element
                .liveValue!.publishableKey;
          } else if (element.type == PAYMENT_TYPE_PAYSTACK) {
            payStackPublicKey =
            element.isTest == 1 ? element.testValue!.publicKey : element
                .liveValue!.publicKey;
          } else if (element.type == PAYMENT_TYPE_RAZORPAY) {
            razorKey =
            element.isTest == 1 ? element.testValue!.keyId.validate() : element
                .liveValue!.keyId.validate();
            ...
            element.isTest == 1 ? element.testValue!.secretKey : element
                .liveValue!.secretKey;
            flutterWaveEncryptionKey =
            element.isTest == 1 ? element.testValue!.encryptionKey : element
                .liveValue!.encryptionKey;
          } else if (element.type == PAYMENT_TYPE_PAYPAL) {
            payPalTokenizationKey =
            element.isTest == 1 ? element.testValue!.tokenizationKey : element
                .liveValue!.tokenizationKey;
          } else if (element.type == PAYMENT_TYPE_PAYTABS) {
            payTabsProfileId =
            element.isTest == 1 ? element.testValue!.profileId : element
                .liveValue!.profileId;
            payTabsClientKey =
            element.isTest == 1 ? element.testValue!.clientKey : element
                .liveValue!.clientKey;
            payTabsServerKey =
            element.isTest == 1 ? element.testValue!.serverKey : element
                .liveValue!.serverKey;
            ...
            element.isTest == 1 ? element.testValue!.publicKey : element
                .liveValue!.publicKey;
            mercadoPagoAccessToken =
            element.isTest == 1 ? element.testValue!.accessToken : element
                .liveValue!.accessToken;
          } else if (element.type == PAYMENT_TYPE_PAYTM) {
            paytmMerchantId =
            element.isTest == 1 ? element.testValue!.merchantId : element
                .liveValue!.merchantId;
            paytmMerchantKey =
            element.isTest == 1 ? element.testValue!.merchantKey : element
                .liveValue!.merchantKey;
          } else if (element.type == PAYMENT_TYPE_MYFATOORAH) {
            myFatoorahToken =
            element.isTest == 1 ? element.testValue!.accessToken : element
                .liveValue!.accessToken;
          } else if (element.type == PAYMENT_TYPE_GOOGLEPAY) {
            // googlepay = element.isTest == 1 ? element.testValue!.accessToken : element.liveValue!.accessToken;
            print("==============================> Google Pay");
          } else if (element.type == PAYMENT_GATEWAY_IYZICO) {
            // googlepay = element.isTest == 1 ? element.testValue!.accessToken : element.liveValue!.accessToken;
            print("==============================> IYZICO Pay");
          }
        });
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    ...
    if (widget.isWallet == true) {
      ...
      savePaymentApiCall(paymentType: PAYMENT_TYPE_RAZORPAY,
          paymentStatus: 'paid',
  ...
  void flutterWaveCheckout() async {
    final customer = Customer(name: getStringAsync(NAME),
    ...
    if (response.status == 'successful') {
      ...
      if (widget.isWallet == true) {
        ...
        savePaymentApiCall(paymentStatus: PAYMENT_PAID,
  ...
  //             .presentPaymentSheet(parameters: PresentPaymentSheetParameters(
  ...
  //               paymentConfirm();
  //             } else {
  //               savePaymentApiCall(paymentType: PAYMENT_TYPE_STRIPE,
  ...
  void payStackPayment(BuildContext context) async {
    ...
    try {
      CheckoutResponse response = await plugin.checkout(
          context, method: method, charge: charge, fullscreen: false);
      ...
      if (response.message == 'Success') {
        if (widget.isWallet == true) {
          ...
          savePaymentApiCall(paymentType: PAYMENT_TYPE_PAYSTACK,
              paymentStatus: PAYMENT_PAID,
              transactionDetail: req,
  ...
  void payPalPayment() async {
    final request =
    BraintreePayPalRequest(amount: widget.totalAmount.toString(),
    ...
    if (result != null) {
      ...
      if (widget.isWallet == true) {
        ...
        savePaymentApiCall(paymentType: PAYMENT_TYPE_PAYPAL,
            paymentStatus: PAYMENT_PAID,
            txnId: result.nonce,
      ...
      "items": [
        {
          ...
          "quantity": 1,
          "currency_id": appStore.currencyCode,
          "unit_price": widget.totalAmount
        }
      ],
      ...
      if (preferenceId != null) {
        ...
        if (result.status == 'approved') {
          if (widget.isWallet == true) {
            ...
            savePaymentApiCall(
              paymentStatus: 'paid',
              paymentType: PAYMENT_TYPE_MERCADOPAGO,
   ...
   void paytmPayment() async {
      ...
      String callBackUrl =
          (isTestType
              ...
              '/theia/paytmCallback?ORDER_ID=' + widget.orderId.toString();
              ...
              if (value['response'] != null) {
                ...
                if (value['response']['STATUS'] == 'TXN_SUCCESS') {
                  if (widget.isWallet == true) {
                    ...
                    savePaymentApiCall(paymentType: PAYMENT_TYPE_PAYTM,
                        paymentStatus: 'paid',
    ...
    Future<void> myFatoorahPayment() async {
      PaymentResponse response = await MyFatoorah.startPayment(
        ...
        errorChild: Center(child: Text(language.failed,
        ...
        request: isTestType
          ...
          language: defaultLanguage == 'ar' ? ApiLanguage.Arabic : ApiLanguage
              .English,
      ...
      if (response.isSuccess) {
        if (widget.isWallet == true) {
          ...
          savePaymentApiCall(paymentType: PAYMENT_TYPE_MYFATOORAH,
              txnId: response.paymentId,
              paymentStatus: 'paid');
        }
    ...
    void onGooglePayResult(paymentResult) {
      ...
      savePaymentApiCall(paymentStatus: PAYMENT_PAID,
          txnId: paymentResult,
          paymentType: PAYMENT_TYPE_GOOGLEPAY,
          transactionDetail: paymentResult);
    }

    _iyzicoPayment() async {
      String token =
      ...
      final response = await http.post(
          Uri.parse(paymentUrl), body: jsonEncode(body), headers: {
                ...
                paymentGatewayList.isNotEmpty
                  ...
                  children: [
                    ...
                    Align(
                      ...
                      child: selectedPaymentType == PAYMENT_TYPE_GOOGLEPAY
                          ? Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: gpay.GooglePayButton(
                          paymentConfigurationAsset: 'gpay.json',
                          paymentItems: _paymentItems,
                          type: gpay.GooglePayButtonType.pay,
                        ...
                        if (selectedPaymentType == PAYMENT_TYPE_MERCADOPAGO) {
                          mercadoPagoPayment();
                        } else if (selectedPaymentType == PAYMENT_TYPE_PAYTM) {
                          paytmPayment();
                        } else
                        if (selectedPaymentType == PAYMENT_TYPE_MYFATOORAH) {
                          myFatoorahPayment();
                        } else
                        if (selectedPaymentType == PAYMENT_GATEWAY_IYZICO) {
                          _iyzicoPayment();
                        }
                      }, width: context.width())
                          .paddingAll(16),
