...
generateInvoiceCall(OrderData orderData) async {
  ...
  final invoice = Invoice(
    supplier: Supplier(
        name: 'Roberts Private Limited',
        address: 'Sarah Street 9, Beijing, Ahmedabad',
        contactNumber: '+91 9845345665'),
    customer: Customer(
    ...
    items: [
      InvoiceItem(
        product:
            '${orderData.parcelType} (${orderData.totalWeight} ${CountryModel.fromJson(getJSONAsync(COUNTRY_DATA)).weightType})',
            ...
            child: Column(
              ...
              children: [
                ...
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children:
                      List.generate(invoice.extraChargeList.length, (index) {
                    ExtraChargeRequestModel item =
                        invoice.extraChargeList[index];
                    return Padding(
                      padding: EdgeInsets.only(bottom: 4),
                      child: buildText(
                        title:
                            '${item.key!.replaceAll("_", " ")} (${item.valueType == CHARGE_TYPE_PERCENTAGE ? '${item.value}%' : '${printAmount(item.value.validate())}'})',
                        value: Utils.formatPrice(countExtraCharge(
                                totalAmount: subTotal,
                                chargesType: item.valueType!,
                                charges: item.value!)
                            .toDouble()),
                      ),
                    );
...
class Utils {
  static formatPrice(double price) =>
      '${printAmount(price.toStringAsFixed(digitAfterDecimal).toDouble())}';
